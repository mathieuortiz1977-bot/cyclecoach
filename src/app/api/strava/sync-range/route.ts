...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[SYNC-RANGE] Fetching activities from ${from.toISOString()} to ${to.toISOString()}`);

    const rider = await getCurrentRiderWithStrava();
    console.log("[SYNC-RANGE] Rider loaded:", rider?.id);

    if (!rider?.stravaConnection) {
      console.log("[SYNC-RANGE] No Strava connection found");
    ...
    console.log("[SYNC-RANGE] Token expired?", isTokenExpired(expiresAt));
    if (isTokenExpired(expiresAt)) {
      console.log("[SYNC-RANGE] Refreshing token...");
      ...
      console.log("[SYNC-RANGE] Token refreshed");
    }
    ...
    while (true) {
      console.log(`[SYNC-RANGE] Fetching page ${page}...`);
      ...
      console.log(`[SYNC-RANGE] Got ${activities.length} activities on page ${page}`);
    ...
    console.log(`[SYNC-RANGE] Total activities fetched: ${allActivities.length}`);
    ...
    console.log(`[SYNC-RANGE] Cycling rides found: ${rides.length}`);
    ...
    for (const activity of rides) {
      try {
        ...
        console.error(`[SYNC-RANGE] Error syncing activity ${activity.id}:`, actErr);
        throw actErr;
      }
    }
    ...
    console.error("[SYNC-RANGE] ERROR:", errorMsg);
    console.error("[SYNC-RANGE] STACK:", errorStack);
    return NextResponse.json({
      error: "Sync failed",
      message: errorMsg
    }, { status: 500 });
